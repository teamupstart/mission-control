...
5. Implement the supported OTel serializer/export path and retry state machine. Cover full/partial success, retryable ambiguity, auth/config/payload failure, stale leases, expiry, quota and clean shutdown. Keep one destination's failures from blocking another; fence changes in consent/endpoint generation. Require HTTPS for credential-bearing non-loopback exports, allow loopback HTTP Collectors and never forward credentials to a changed redirect destination. Exclude outbound exporter environment/credentials from child agents.
...
Transport fixtures must reject a credential-bearing remote HTTP export before transmission, accept the supported loopback HTTP case, validate HTTPS normally and prove a cross-destination redirect cannot carry credentials.
...
Review reconciliation: made the credential-bearing remote HTTPS requirement and redirect behavior explicit in P1, P4 and Phases 1/2. Exporter checks and Settings validation now share one contract; no audience, durability or dashboard scope changed.
