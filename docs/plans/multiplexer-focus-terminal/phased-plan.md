...
| `TerminalDeps` carries only the two registries - no availability dependency. | `src/server/terminal/registry.ts:118-121` | **Corrects the plan.** `binUnavailableReason(spec, label, deps)` needs a `BinAvailabilityDeps` as its third argument, which `TerminalDeps` does not supply. The fix is to extend `TerminalDeps` and give `defaultTerminalDeps` `installed` / `unsupported`, mirroring `defaultTerminalTargetDeps` (`src/server/terminal/targets.ts:48-54`); the helper is then called exactly as `raiser` already calls it (`:75-78`), not replaced. Recorded in the phase file. |
