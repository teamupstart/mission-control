...
- Bindings: `workflow_bindings` with scalar `session_cwd`/`session_repo_root` and `CREATE UNIQUE INDEX idx_workflow_bindings_active_note ON workflow_bindings(note_key) WHERE state='active'` (`src/server/db.ts:438-455`; index at 454, verified). One active binding per session note is the invariant to widen, not delete.
- Runs: `workflow_runs.inspector_pr_key` / `inspector_head_sha` scalars (`db.ts:468-469`); `workflow_submissions.pr_head_sha` (`db.ts:514`). Gate state `WorkflowInspectorGateState` (`src/shared/workflow.ts:1148-1160`) is one PR per gate; the pin-switch refusal is `src/server/workflows/manager.ts:2495-2506` (`inspector_pr_switch_refused`); candidate resolution reads the scalar `session.prUrl` (`manager.ts:2489-2493`, `:1507-1509`).
- The `pull_request` session action: adapter at `src/server/workflows/session-action-adapters.ts:284-422`; repository facts are supplied by the manager on `SessionActionAdapterContext` (`:68-80`); repo identity via `readWorkflowRepositoryHead`/`readWorkflowRepositoryId` (`src/server/workflows/context.ts:546-616`, `--git-common-dir`). `adoptedPullRequestsForAction` (`manager.ts:3783-3814`) already resolves a repository identity per distinct `pr.repoRoot` - the one existing multi-repo-aware reader.
...
- Merge veto: `blocksMerge(prKey)` (`manager.ts:1579-1601`) holds YOLO merge while a run's gate pins that key.
- UI: `WorkflowChip`/`workflowRunTone`/`workflowRunLabel` (`src/web/components/session-bits.tsx:142-200`); run surfaces under `src/web/workflows/` (`run-model.ts` is the vocabulary owner). `useEventStream.ts` handles `workflow_run_upsert`/`_remove` (`:241-251`).
...
2. **Run creation** (`src/server/workflows/manager.ts`): at trigger time, evaluate the shared changed-set predicate per task repo entry (head vs `base_sha`, read from the pinned worktrees); create one run per changed repo, each bound to its repo entry; a single-repo session creates exactly one run through the unchanged path. Unchanged repos are skipped silently and never surface a run.
...
- Unit: binding uniqueness per `(note_key, repo)` incl. upgrade/backfill; run creation skips unchanged repos and creates one run per changed repo; per-repo evidence capture (facts come from the run's worktree); submission routing (explicit, single-active implicit, head-moved inference, ambiguous refusal); serialization (second run's delivery queues, dequeues on completion, survives restart); `blocksMerge` membership; single-repo regression across the existing workflow suites (`test/workflow-inspector-gate.test.ts` and siblings updated deliberately).
