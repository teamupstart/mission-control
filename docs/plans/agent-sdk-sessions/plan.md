...
|---|---|---|---|
| Surviving daemon death | tmux keeps the agent running if the daemon crashes or restarts; the sweep re-adopts it | SDK subprocesses are daemon children, so a restart interrupts the process; [auto-continue recovery](../sdk-session-auto-continue/plan.md) resumes the same conversation and any durably unfinished turn | Low residual risk; see the recovery plan for the at-least-once boundary |
...
- **Persists** to `sdk_sessions` (id TEXT PRIMARY KEY NOT NULL, agent,
  ...
  `turn_in_progress`, timestamps). The
  [auto-continue recovery plan](../sdk-session-auto-continue/plan.md) owns the
  interrupted-turn contract and migration; there are no REFERENCES clauses (the ensemble
...
- **Resumes on startup**: restore rows, relaunch handles with `resume`, register the
  ...
  ones. Interrupted-turn handling follows the
  [auto-continue recovery plan](../sdk-session-auto-continue/plan.md). A session whose
  resume fails is registered, marked `exited`, and evicted through the normal path so
