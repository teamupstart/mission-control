...
| `MISSION_JIRA_BIN` | `jira` | the [Jira CLI](dispatch-and-backlog.md#jira) used by every `jira` subprocess: a Jira task source's sweep, and the comment, remote link and transition its [write-back](dispatch-and-backlog.md#jira) sends. Mission Control passes fixed arguments and stores no Jira credential; the CLI keeps its own. Point it at a wrapper script to change how it is invoked - and note that this is the seam the browser suite redirects at a fake, because a write-back **moves an issue** on somebody's board and an unfaked binary on a machine that ran `jira init` would do it for real |
| `MISSION_PRODUCT_ISSUES_REPO` | `teamupstart/mission-control` | the one public GitHub repository [product reports](ui.md#report-product-feedback) may target, from the dashboard's Feedback form and from the agent tool alike. The value must be an exact `owner/name` and exists for downstream forks and isolated tests. A report request cannot override the repository, labels, source, assignee, project, or milestone. The target must already carry all eight labels - `bug`, `feature-request`, `documentation`, `usability`, `other`, `status:needs-triage`, `source:dashboard`, `source:agent` - or preflight refuses and names the missing ones |
...
| `MISSION_TASK_SOURCE_WRITEBACK_TICK_MS` | `20000` | Task sources: how often the write-back worker wakes to deliver what a swept task owes its upstream item. Floored at `5000` |
| `MISSION_TASK_SOURCE_WRITEBACK_SETTLE_MS` | `300000` | Task sources: how long the write-backs that assert a task finished - the completion comment and the optional resolve - wait before either may be delivered. The pause exists because a completion inferred from an idle agent can be reversed; the worker re-checks the live task when the window expires and cancels both if the work resumed. The pull-request comment is not delayed, because a pull request that opened stays opened. A value that is not a finite number falls back to the default rather than becoming `NaN` |
...
| `MISSION_PI_EXTENSION` | `dist/pi-integration/extension.js` | Pi integration build target and daemon artifact probe override; use `extension.js` beside its MCP bridge and manifest. Building does not install; Setup copies and verifies an app-owned generation |
...
Skills snapshots include the catalog-wide default and explicit row choices. Restoring an older
snapshot with no catalog default keeps its previous opt-in meaning, so absent row choices stay off.
