...
- **One file owns almost all of it.** `src/web/workflows/WorkflowRuns.tsx` is 3,915 lines and contains every section in scope: `SubmissionImageEvidence` (lines 218-303), `SubmissionEvidenceReadiness` (306-440), the Review worklist section (2453-2504), the Inspector gate (2505-2620), the Foreman completion claim (2622-2638), the deliveries (2641-2730) and the captured context (2731-2853). Roughly 620 existing lines are restructured. This is the dominant fact for phase boundaries: phases cannot be made concurrent by topic because they share one file.
- **The tab component exists.** `.workflow-tabs` is already styled (`src/web/styles.css:14249`) and used by `src/web/pipelines/RunsKindTabs.tsx` with a real `role="tablist"`. It takes a badge (`.workflow-tab-badge`). No new component family is needed, only a container.
...
- **The worklist carries a tour target.** `useTourTargetRef<HTMLElement>("library:run-worklist")` is attached to the Review worklist section (line 2453) and `src/web/tour/tours/library.ts:311` steps onto it. The worklist must stay the default pane and keep that ref on a node that is in the DOM when the tour runs.
...
- **Inspector findings are not in the worklist.** `runChangeWorklist` (`src/web/workflows/run-model.ts:2104`) is built only from Persona attempt verdicts (`if (!attempt.persona) continue`). A finding has never appeared there, so the Completion pane is the only surface that shows them and folding them into the worklist would change what that list means.
- **A cross-reference will go stale.** `src/web/workflows/run-actions.ts:818` reads "Turn it back on from Open GitHub Inspector settings, in GitHub Inspector final gate below." Once the gate is a tab that sentence is wrong. `test/workflow-runs-render.test.ts:878` asserts it.
...
- **Existing coverage that must move with the sections.** `test/workflow-runs-render.test.ts` (2,865 lines) asserts the gate heading, the context headings and the disabled-Inspector sentence. Four e2e specs assert the affected surfaces: `workflow-session-action-run.spec.ts:301` on the "Deliveries to the session" heading, `workflow-round-limit-grant.spec.ts:460` on the "Last workflow observation" and "Current Inspector" regions, plus `workflow-image-evidence.spec.ts` and `workflow-evidence-readiness.spec.ts`.
