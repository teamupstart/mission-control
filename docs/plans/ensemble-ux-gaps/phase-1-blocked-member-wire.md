...
- Partition test in `ensemble-needs-input.test.ts`: a `submitted` member with a pending review counts in `membersNeedingInput` and NOT in `membersReady`; resolving the review moves them (1 blocked / 0 ready becomes 0 / 1); `membersOut + membersNeedingInput + membersReady <= launchedMembers` holds across every driven state.
