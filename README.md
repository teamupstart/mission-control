...
leaves the pass at any point and keeps what it has. While the questions are up, dropping an
image anywhere on the window attaches it to that same task. It ships off; the switch turns it
on, and so does **Settings → Dispatch**, which is where ⌘K finds it by name. See
