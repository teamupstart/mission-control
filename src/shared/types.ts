...
export interface NmRunSummary {
  /**
   * The run's own id (a ULID from `axi status`). Identifies the run across polls
   * independently of its branch, which several successive runs can share - so
   * dismissing one run never gags the next one on the same branch.
   */
  id: string;
