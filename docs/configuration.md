...
| `MISSION_CLAUDE_TRANSPORT` | `print` | Claude's headless wire protocol: `print` uses `claude -p`; `sdk` uses one fresh Agent SDK `query()` with deterministic setting sources and AbortController cancellation. Tool-less calls keep every tool disabled; Inspector reviews and replies keep their existing `Read,Grep,Glob` grant and provider-enforced deny rules. The stored `llm.claudeTransport` choice wins where set, then this environment fallback, then `print`. The daemon resolves that ladder and shares the result with the separate Foreman worker over `/api/llm/status`, so Foreman's review, queue verify, backlog planner, and Tier 1 triage calls follow the same choice without opening the database |
...
| `MISSION_MCP_SERVER` | app's `dist/mcp/server.mjs` | path to the bundled MCP server that dispatched sessions are pointed at through [the ask channel](sessions.md#the-ask-channel)'s `--mcp-config`. If the path doesn't exist the channel is skipped entirely and the session keeps Claude's built-in menu |
