...
5. **Progress dots** - one shared leaf `EnsembleProgressDots` in `session-bits.tsx` (per the shared-leaf rule): renders `maxMembers` squares colored by member disposition derivable from summary counts (ready = submitted, launched-not-ready = working, needsInput count = blocked, remainder = pending). Used by the board cluster header and the run list row.
