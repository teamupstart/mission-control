...
- `src/server/workflows/persona-workload/executor.ts` owns the local reference supervisor and accepts an injected `RepositoryArtifactMaterializer`. Materialization requests include submission, workload, Workflow attempt, artifact locator, and artifact digest identity. Materialization and provider execution are raced against the executor-owned deadline even when an injected dependency ignores cancellation, and a lease that arrives after timeout is released asynchronously. The daemon-owned prompt, submission images, text evidence, and LLM call accounting remain in the workload contract.
