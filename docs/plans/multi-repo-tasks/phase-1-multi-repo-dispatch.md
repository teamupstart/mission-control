...
- `provisionWorktree`: `src/server/dispatcher.ts:760` (pool arm 799-865, git fallback 867-893, branch `harness/<slug>-<shortId>`); `teardownWorktree`: `dispatcher.ts:925` (treehouse return without `--force`, git fallback removes + deletes only `harness/`-prefixed branches). `verifyPinnedBase` at 696 demands a full 40-char sha. Runtime branch at dispatcher.ts:206; SDK arm passes `cwd: wt.path, gitRoot: wt.path` (~425-441).
