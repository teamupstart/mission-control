...
- `src/server/workflows/persona-workload/executor.ts` owns the local reference supervisor and accepts an injected `RepositoryArtifactMaterializer`. Materialization requests include submission, workload, Workflow attempt, artifact locator, and artifact digest identity. Materialization and provider execution are raced against the combined workload-cancellation and deadline signal even when an injected dependency ignores cancellation, and a lease that arrives after termination is released asynchronously. A terminal cancellation event is emitted without waiting for a non-cooperative provider, but the live provider promise retains ownership of its repository lease and attempt directory until it settles. Ordinary cancellation retains a bounded 100 ms reconciliation interval for a provider result that carries audit and LLM usage accounting; deadline expiry adds no reconciliation time. The executor places the frozen Persona guidance in an explicit provider-visible boundary shared by both adapters. The daemon-owned prompt, submission images, text evidence, and LLM call accounting remain in the workload contract.
