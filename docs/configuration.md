...
| `MISSION_TASK_SOURCE_WRITEBACK_SETTLE_MS` | `300000` | Task sources: how long a resolve waits after a task completes before it may close or transition the item. The pause exists because a completion inferred from an idle agent can be reversed; the worker re-checks the live task when the window expires, and cancels the resolve if the work resumed |
