...
5. **Progress dots** - one shared leaf `EnsembleProgressDots` in `session-bits.tsx` (per the shared-leaf rule): renders `maxMembers` squares from the summary's PARTITIONED member counts (Phase 1's contract: `membersOut`, `membersNeedingInput`, `membersReady` never count the same member twice) - `out`, `blocked`, `done` (`membersReady`, NOT the artifact-count `readyArtifacts`), `working` (`launchedMembers` minus the three, floored at 0), `pending` (remainder to `maxMembers`). Because the inputs partition, the dot total is exactly `maxMembers` by construction; the component still clamps defensively and never renders more. The counts are aggregates, not an ordered member list, so the dots claim only how MANY members are in each disposition, never which ordinal - the tooltip says so. Used by the board cluster header and the run list row.
