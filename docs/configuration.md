...
| `MISSION_CLAUDE_TRANSPORT` | `print` | Claude's headless wire protocol for daemon-side calls: `print` uses `claude -p`; `sdk` uses one fresh Agent SDK `query()` with deterministic setting sources and AbortController cancellation. Tool-less calls keep every tool disabled; Inspector reviews and replies keep their existing `Read,Grep,Glob` grant and provider-enforced deny rules. The stored `llm.claudeTransport` choice wins where set, then this environment fallback, then `print`. Foreman remains on `print` in this phase |
