import { PRIMARY_ACTION_IDS, PRIMARY_FEATURES, isPrimaryUseAction } from "./primary-actions.ts";
import { z } from "zod";
...
export const ACTION_RESULT_SCHEMA = z.object({
  feature: z.enum(["workflow", "persona", ...PRIMARY_FEATURES]),
  action: z.enum([...WORKFLOW_ACTION_ROUTES.map((r) => r[2]), ...PRIMARY_ACTION_IDS] as [string, ...string[]]),
  outcome: z.enum(["applied", "refused", "failed", "pending", "cancelled"]),
  observation: z.enum(["initial", "applied_update", "outcome_update"]).default("initial"),
...
export const ACTION_METRICS: TelemetryMetricDefinition[] = [
  { ...base, name: "mission.feature.used", dimensions: ["feature", "actor"],
    description: "Successful use of Phase 5 operations; configuration is separate from adoption",
    contribution: (f, e) => isPrimaryUseAction(String(f.feature), String(f.action))
      && f.outcome === "applied"
      ? { dimensions: { feature: String(f.feature), actor: e.actor.kind }, value: 1 } : null },
  { ...base, name: "mission.action.count", dimensions: ["feature", "action", "outcome", "actor"],
    ...
    contribution: (f, e) => f.observation === "initial"
  ...
  { ...base, name: "mission.workflow.interventions", dimensions: ["action", "intent", "cause"],
    contribution: (f, e) => ["workflow", "persona"].includes(String(f.feature)) && f.outcome === "applied" && e.actor.kind === "human"
