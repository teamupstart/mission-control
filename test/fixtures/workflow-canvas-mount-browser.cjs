...
app.whenReady().then(async () => {
  try {
    ...
    while (Date.now() < deadline && result === null) {
      result = await window.webContents.executeJavaScript("window.__workflowCanvasResult ?? null");
      if (result === null) await new Promise((resolve) => setTimeout(resolve, 50));
    }
    if (result === null) throw new Error("Workflow canvas mount did not settle");
