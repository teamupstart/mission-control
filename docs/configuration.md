...
| `MISSION_CLAUDE_TRANSPORT` | `print` | Claude's headless wire protocol for daemon-side tool-less calls: `print` uses `claude -p`; `sdk` uses one fresh Agent SDK `query()` with tools disabled, deterministic setting sources, and AbortController cancellation. The stored `llm.claudeTransport` choice wins where set, then this environment fallback, then `print`. Foreman and the Inspector's granted reviews remain on `print` in this phase |
