...
- App owns the checkbox's in-flight flag and transient save error, so dismissal or route changes cannot discard a pending result. The callback path is TourPicker to App's save handler to `updateUiConfig({ showToursOnStartup: next })` to `api.setUiConfig` (`PUT /api/ui/config`). A false result follows the existing per-field optimistic rollback. Read the restored value from `useUiConfig`; do not keep a second preference copy. Reopening during a save keeps the checkbox disabled.
- Report failure with **Could not save your tour preference. Your previous setting still applies.** Show it once inline with `role="alert"` while the picker is open. After dismissal, retain it in App and render the proposed `TourPreferenceNotice` in App's existing banner area, after SetupBanner and before AppPageShell. If another overlay or tour owns the screen, wait until it is idle. A failure shown inline also survives subsequent dismissal. Never reopen the picker, change the route, or move focus just to report the error.
- The notice is a small feature-owned renderer using `.app-banner.app-banner-error`, `.app-banner-copy`, and `.app-banner-actions`; its message has `role="alert"`. **Browse tours** uses the same guarded picker opener and transfers the error inline; **Dismiss** clears only the transient error. Retain it until dismissal or the next explicit save attempt, without a timeout. Retry by changing the checkbox again. No generic toast service, desktop notification, hidden retry, or additional persistence store is needed.
...
| `src/web/App.tsx`, `src/web/components/SetupBanner.tsx`, `src/web/components/SettingsRestoredBanner.tsx`, `src/web/styles.css` | App already renders banners outside AppPageShell on every route. Add App-owned preference save/error state and a small proposed `TourPreferenceNotice.tsx` renderer in that region, reusing the existing banner styles and narrow-screen stacking. This is the owned post-dismissal failure surface. |
| `src/web/useNotifier.ts` | Its private notify helper emits permission-gated desktop notifications for attention transitions. No general dashboard-toast API was found. Do not repurpose this hook or App's launcher-specific error flash for preference failures. |
