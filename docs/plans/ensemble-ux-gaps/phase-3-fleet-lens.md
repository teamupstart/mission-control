...
2. **Props** - add `ensembleSummaryByRun: Map<string, EnsembleSummary>` to `SessionViewProps` and `viewProps` in App; App already holds the summaries (`useEventStream`). Key it by run rather than session because one cluster header represents several sessions; a missing entry is a first-class fallback while the member link and summary arrive on separate SSE edges.
...
4. **BoardView** - group the per-column `.map` by the cluster spans: members render inside a `.board-cluster` frame with a header row (`⧉ title · strategyLabel · ensembleStageWord(summary) · progress dots`). Its rollup is local to the repeated frame: solid **N needs you** only when one of that frame's members is blocked, otherwise outlined **N elsewhere** when the blocked member is in another tone column. Header click calls `onOpenEnsemble(runId)`. The frame is presentational (a bordered wrapper + header div); tiles keep their exact props and DOM order; dragover passes through (no handlers on the wrapper).
5. **Progress dots** - one shared leaf `EnsembleProgressDots` in `session-bits.tsx` (per the shared-leaf rule): renders `maxMembers` squares from the summary's PAIRWISE-DISJOINT member counts (Phase 1's contract: `membersOut`, `membersNeedingInput`, `membersReady` never count the same member twice; a plain working member is in none of them) - `out`, `blocked`, `done` (`membersReady`, NOT the artifact-count `readyArtifacts`), `working` (`launchedMembers` minus the three, floored at 0 - the remainder Phase 1's contract says the consumer computes), `pending` (remainder to `maxMembers`). Because the inputs are disjoint, the dot total is exactly `maxMembers` by construction; the component still clamps defensively and never renders more. The counts are aggregates, not an ordered member list, so the dots claim only how MANY members are in each disposition, never which ordinal - the tooltip says so. Used by both cluster-header densities and the run list row.
...
7. **Marks** (`session-bits.tsx`) - `ensembleMemberStateLabel` prefers `needsInput` ("needs an answer", attention tone) over the status ladder; `EnsembleChip` gains the run-progress suffix ("· 3/5 in") from summary `membersReady` / `maxMembers` only when the summary map is reachable. Where only the member link is available it renders no suffix rather than inventing another progress vocabulary. `EnsembleRailMark` gains the tone class; `EnsembleTileFlag` renders the member ordinal over the roster (`E 3/5`), not run progress. Every tooltip uses `maxMembers` as the candidate denominator; a reachable summary adds the shared stage word and `membersReady` progress.
...
9. **Run list rows** (`EnsembleRuns.tsx`) - add `EnsembleProgressDots` and a `membersReady/maxMembers in` counts line, adding `launchedMembers` only while it is short of the roster; "N needs you" when `membersNeedingInput > 0`.
...
- Board render test (renderToStaticMarkup): cluster header renders title/stage word/dots; tiles inside keep order; a blocked member's own frame shows "1 needs you" while sibling columns show "1 elsewhere".
...
- 2026-07-27 (PR #282, Inspector round 1): the rail cluster header keeps its **density exception** - title, stage word, dots and a compact attention count, but NOT the strategy label. The Inspector was right that the shipped docs promised it; the docs were the wrong side of the disagreement and were corrected. Measured at the rail's real widths before deciding: at the 260px floor a `Best of N ·` prefix cuts the run's own name to 4-9 characters ("Stabilize parser rollout" becomes "Stabi…"), and even at the 340px maximum it ellipsizes titles that fit today. The strategy is the one header field also present on every member row's chip, and it stays in the header's hover copy. Downstream phases inherit this: the two densities are not the same field list.
- 2026-07-27 (PR #282, review gate): three deviations from steps 4-7 as written, all narrower rather than wider. (a) The repeated frame's rollup is LOCAL: a solid **N needs you** only where a member of that frame is blocked, an outlined **N elsewhere** otherwise - one badge for both put an amber call to action over the `gone` column's failed candidate. (b) The rail header gained the same attention count as a compact `!N`, because each tone section draws its own copy and the tone class alone left a sibling blocked in another section invisible in the Console. `ensembleAttentionInFrame` is the one place that arithmetic lives. (c) `EnsembleChip` renders its progress suffix ONLY when a summary is reachable; step 7's "the link's counts suffice" was declined, because `launchedMembers` over `maxMembers` is a second progress vocabulary and the downstream handoff forbids one. The tile flag's `E 3/5` is the member ordinal over the roster, not progress.
- 2026-07-27 (PR #282): `ensembleClusterHeadline` is the headers' accessible name as well as their hover copy, and an `aria-label` REPLACES its element's whole subtree - so it has to state what the header's nested children would have said for themselves. It previously dropped three things: the run's name and the attention count before the summary arrived (both come from `blockedHere`, which is derived from member links), and every progress-dot state but "submitted", because the dots' own `role="img"` name is exactly what a labelled ancestor discards. The roster clause is `ensembleDotSummary`, the same sentence the dots use, so one row cannot be described two ways. **Downstream phases: a cluster-header child that carries its own accessible name needs that name folded in here, not left nested.**
- 2026-07-26 (round 7): dots moved onto Phase 1's partitioned counts (`membersReady` instead of `readyArtifacts`) after the Inspector showed a submitted-and-blocked member was double-counted and the dot total could exceed `maxMembers`. Exclusivity now lives at the source; the component only clamps defensively.
