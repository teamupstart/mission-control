...
test("an invalid named credential file never falls back to the legacy credential", () => {
  ...
  const insecure = credentialFile("insecure.json", { credential: current, expiresAt: 101 });
  chmodSync(insecure, 0o644);

  for (const path of [invalid, expired, malformed, insecure, join(root, "missing.json")]) {
    assert.equal(
      readPipelineCallerCredential({
        [PIPELINE_CALLER_CREDENTIAL_FILE_ENV]: path,
        [PIPELINE_CALLER_CREDENTIAL_ENV]: legacy,
