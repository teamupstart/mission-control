...
| A slow probe stalls the page | Probes run concurrently; the subprocess-costing ones (`gh auth status`, the conductor probe) sit behind the short-lived cache the Settings route already uses, and each row renders `unknown` with a reason rather than blocking. |
