...
Repository citations have an explicit versioned rollout gate. The daemon, workload executor, and verdict parser must all advertise the same repository-evidence protocol capability before a read-enabled workload launches. A missing or mismatched capability is an infrastructure failure and cannot persist a repository citation. Mission Control has one durable database reader, so the database migration and all three strict evidence schemas ship atomically in the same application release. After the first repository citation is persisted, opening that state with an older application is unsupported; rollback restores the verified pre-migration database recovery point instead of opening upgraded state with an older reader.

Migration and mixed-version tests open representative pre-feature databases, including custom Personas, built-in workflow history, published custom workflows, runs, attempts, and LLM call audit rows. They prove that all old records parse as `none`, repeated migration is safe, old versions keep identity, identical republish is idempotent, a changed resolved Persona snapshot creates a new immutable workflow version, old eight-kind verdicts remain readable, repository verdicts round-trip through every current schema, and an older or capability-mismatched executor/parser is rejected before dispatch or verdict persistence. A frozen pre-feature reader fixture must still open the verified pre-migration recovery copy and must reject, never partially decode, an upgraded repository verdict.
...
- Evidence handles are minted only for actually returned allowed items; line, byte, and diff ranges round-trip exactly, while forged, cross-attempt, mismatched-range, denied, failed, cancelled, and conflicting duplicate handles fail without persisting or reconstructing a quote.
...
- Snapshot capture, portable reconstruction, and rollback under checkout mutation, Git failure, database failure, digest mismatch, every prepare/promote/discard crash window, restart reconciliation, retention, and cleanup retries.
...
- Mixed-version repository-evidence capability negotiation fails before provider launch or durable verdict write; current readers retain all old evidence and exact line/byte/diff repository ranges.
