...
import { claudeSpendReport } from "../src/server/llm/claude.ts";
import {
...
// caller has given up. Every query below is scripted and no agent binary or model is used.
...
test("a grant is refused before the binary is resolved or query is called", async () => {
  const fake = fakeDeps([SPEND_FRAME]);
  await assert.rejects(
    runClaudeSdkOneShot("review this diff", {
