...
6. **Completion quorum** (`src/server/tasks.ts`): for a multi-repo task, `reconcileMergedTasks` completes only when every repo in the changed set has a merged PR. Changed set = repos with an episode PR, union repos whose worktree head differs from `base_sha`, evaluated over the primary and every secondary alike - the primary's baseline read from `tasks.base_sha` (phase 1), the secondaries' from their `task_repos` rows (heads read at reconcile time from the still-pinned worktrees; a torn-down worktree with no episode PR counts as unchanged). The primary must be in this set or the quorum is unsound: a task whose primary changed but opened no primary PR would otherwise complete on a merged secondary PR alone, shipping the primary work unmerged. Closed-unmerged does not satisfy. `outcome` lists every merged PR; `outcomeUrl` stays the primary's PR. Single-repo tasks keep today's exact behavior.
...
- Unit: multi-URL sniffing (one command opening two PRs yields both); per-repo acceptance and per-repo refusal of a replacement; poll-target fan-out; quorum - completes on all-merged, holds on one-open, holds on closed-unmerged, exempts unchanged repos, upgrades `failed`/`cancelled` per `completableByMerge`, and **holds when the primary's head moved off `tasks.base_sha` but opened no primary PR while a secondary PR merged** (the case a secondaries-only baseline would have completed wrongly); single-repo regression (existing tests such as `test/task-durable-merge.test.ts`, `test/task-completion-reconciler.test.ts` updated deliberately, not deleted); projection rendering via `renderToStaticMarkup` for the per-repo PR list.
...
- The changed-set rule (episode PR present, or head differs from `base_sha`) - phase 3 reuses the same rule for run creation; the shared predicate lives in `src/shared` and both consumers import it. The predicate covers the primary as well as the secondaries, taking the primary's baseline from `tasks.base_sha`; a version that iterates only `task_repos` rows silently excludes the primary and breaks both consumers.
...
- 2026-08-10 (review): the changed-set rule and the quorum now state explicitly that they cover the primary, reading its baseline from the `tasks.base_sha` column phase 1 adds. As originally written, `base_sha` existed only for secondaries, so a primary that changed without opening a primary PR was invisible to the changed set and the quorum could complete the task on a merged secondary PR while primary work sat unmerged. Section 7 gains a test for exactly that case.
