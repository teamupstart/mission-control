...
// The button also announces its badge and draft status. Anchor the built-in badge so a
// copied workflow cannot match; copies are addressed by their exact visible heading.
...
async function duplicateBuiltin(page: Page, daemon: DaemonHandle): Promise<string> {
  ...
  await page.getByRole("button", { name: BUILTIN_BUTTON }).click();
  ...
  await expect(page.getByRole("button").filter({ has: page.getByText(COPY, { exact: true }) })).toBeVisible({
    timeout: 15_000,
  });
