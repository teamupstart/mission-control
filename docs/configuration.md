...
| `MISSION_CLAUDE_TRANSPORT` | `print` | Claude's headless wire protocol for tool-less calls: `print` uses `claude -p`; `sdk` uses one fresh Agent SDK `query()` with tools disabled, deterministic setting sources, and AbortController cancellation. The stored `llm.claudeTransport` choice wins where set, then this environment fallback, then `print`. The daemon resolves that ladder and shares the result with the separate Foreman worker over `/api/llm/status`, so Foreman's review, queue verify, backlog planner, and Tier 1 triage calls follow the same choice without opening the database. The Inspector's granted reviews remain on `print` in this phase |
