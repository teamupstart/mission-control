...
- 2026-08-09 (implementation): four deviations, none of which move phase 3's frozen contract (section 9).
  ...
  2. **A transaction that throws AFTER the issue exists is an `unknown-outcome`, not an error.** Step 5.2.6 tabled only the attach-refused case. A `transaction()` that throws (a locked database) rolls both writes back while the issue is already upstream, and letting that fall through to a 500 would leave a retryable-looking button over a created issue. It maps to 504 with `outcomeUnknown: true` - the same "do not blind-retry" answer, for the same reason - and the error names the issue. No phase 3 change: it is a status code phase 3 already handles.
