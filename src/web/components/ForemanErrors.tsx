  ...
  return (
    <article className="foreman-error-issue">
      ...
      <p className="foreman-errors-hint">
        <span title={new Date(issue.firstSeenAt).toLocaleString()}>First seen {relativeTime(issue.firstSeenAt)}</span>
        {" · "}<span title={new Date(issue.lastSeenAt).toLocaleString()}>Last seen {relativeTime(issue.lastSeenAt)}</span>
      </p>
      {issue.sessions.length > 0 && (
      ...
      <div className="foreman-error-actions">
        {issue.runner && <button type="button" className="btn btn-ghost" onClick={onOpenModels}>Open model settings</button>}
