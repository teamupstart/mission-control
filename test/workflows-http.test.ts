...
test("the shipped workflow is readable through the existing workflow routes", async () => {
  ...
  assert.equal(shipped.publishedVersion, 11, "the newest shipped version is the current one");
  ...
  assert.equal(detailBody.workflow.currentVersionId, `${BUILTIN_ID}@11`);
  ...
  assert.equal((await request(`/api/workflows/${BUILTIN_ID}/versions/12`)).status, 404);
});
