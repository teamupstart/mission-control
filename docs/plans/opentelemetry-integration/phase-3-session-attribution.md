...
7. Emit task outcome and per-repository PR-created/associated/updated/merged/closed observations from verified owners. A prose claim or association with an existing PR is not verified creation. At first verified association, durably retain the minimal opted-in task/episode/repository context and local-only PR lookup reference through P1, before source ownership disappears. Extend the existing daemon PR poller with a distinct telemetry observation reason, sharing URL fetches with operational targets. Feed verified late results into durable facts and a cohort-input lookup; observation-only results cannot enter `mergedPrFor`, complete tasks or release dependencies. Keep existing operational eligibility for URLs with both reasons. Bound lifetime, poll work, privacy and coverage as P2 specifies; count a multi-repository task once and preserve unknown PR visibility.
...
Add a late-merge fixture that captures an open PR, removes/rotates session ownership through the real invalidation path, restarts, then returns a verified merge from the shared poller. Assert the cohort-input lookup receives exactly one late-delivery fact with original attribution while task status, session-end evidence and selection-time dependency edges stay unchanged. Cover edges selected before/after invalidation, a URL with both polling reasons, repeated poll results, expiry and disabled capture. Run relevant `test/task-dependencies.test.ts`, `test/task-completion-reconciler.test.ts`, `test/task-durable-merge.test.ts` and `test/task-multi-session.test.ts` cases to guard the existing operational boundary. Phase 6 consumes this fixture to test final cohort revision.
...
Review reconciliation: confirmed the invalidation gap against `db.ts` and its completion/polling readers (99% confidence from the explicit deletion and source contract). Assigned observation retention, shared-poller integration and the ownership-removal fixture to Phase 3. Phase 6 consumes late facts and verifies cohort revision; neither phase changes operational completion or dependency-release policy.
