...
- Sniffing: `src/shared/pr-command.mjs` - `pullRequestUrlIn` returns the FIRST match only (`:51-53`); `PR_URL_RE` at 47. Hook carriers: `hooks/harness-hook.mjs:31-59` (scalar `prUrl`, `prCreated`), `hooks/codex-hook.mjs:23`. SDK drivers: `src/server/harness/claude/sdk.ts:745-775`, `src/server/harness/codex/sdk.ts:1064-1088`.
- Session PR scalars: `src/shared/types.ts:488-501,559`; writers `registry.applyHook` (~1699), `applyDriverPrCreated` (~1586), `reconcilePrs` (~3129) - all overwrite.
- Episodes: `session_work_episodes` (PK `session_id`) and `task_work_episode_bindings` (PK `task_id`, unique per session) at `src/server/db.ts:183-235`, scalar `branch`/`pr_url`/`pr_head_sha`/`merged_at`. `acceptPrForEpisode` refuses a second PR (`registry.ts:2938`); the SQL guard is `WHERE ... (pr_url IS NULL OR pr_url = ?)` (`db.ts:3317-3340`). `historical_task_work_episode_bindings` is the only existing many-PRs-per-task shape, reachable only sequentially via rollover.
- Adoption ledger: `inspector_prs` keyed `owner/repo#number` with `repo_root`/`cwd`/`observed_*` (`db.ts:949-986`) - already multi-PR. `announcePrOpened` (`registry.ts:1625-1643`) deliberately allows a session's second PR.
- Branch poller: `src/server/pr.ts` - one `gh pr list --head <branch>` per distinct cwd (`:261-270`); targets from `registry.prPollTargets` (~2970: one cwd+branch per session). URL poller `queryPrUrl` (`:150-163`) needs no cwd; URL harvesters `dependencyPrPollTargets`/`taskPrPollTargets` (~3142-3218) are already flat URL sets.
- Completion: `completableByMerge` (`registry.ts:5416`), `reconcilePrMerges` -> `markWorkEpisodeMerged` (`registry.ts:2565`, SQL `db.ts:3387-3435`), `TaskManager.reconcileMergedTasks` (`tasks.ts:581-607`) - one merged PR (`mergedPrFor`, newest `mergedAt`) completes the task. `outcome`/`outcomeUrl` scalars (`types.ts:1575-1576`).
- UI: single outcome link on `SessionCard.tsx:358-392` and `ConsoleDetail.tsx:318-345`; `prChipView` (`session-bits.tsx:1096`) is the one seam for the session chip (unchanged here); `LabelChips` (`session-bits.tsx:1633`) is the chips-with-overflow primitive; report repo line `ReportPanel.tsx:109-117`.
...
8. **UI** (`src/web/components/SessionCard.tsx`, `layouts/ConsoleDetail.tsx`, `ReportPanel.tsx`): the task area renders one PR line per repo (repo short name, PR state chip, link), using the existing chip primitives; single-repo tasks render exactly today's markup. Report lines list the repo set.
