  ...
  for (const builtin of BUILTIN_WORKFLOWS) {
    ...
    for (const version of builtin.versions) {
      ...
      for (const node of personaNodes) {
        ...
        const frozenIntent = builtin.definition.id === builtinWorkflowId(NO_MISTAKES_REVIEW_WORKFLOW_SLUG)
          && version.version <= 16
...
// ---- Version 11: code design joins the parallel code-review stage ----

const NO_MISTAKES_V11_NODES = [
  ["nmr-session", "session", 60, 60],
  ["nmr-check-typecheck", "check", 340, 60],
  ["nmr-check-test", "check", 340, 230],
  ["nmr-build-join", "all_pass", 620, 145],
...
test("version 11 reviews design alongside risk and quality in stage 3", () => {
  const builtin = noMistakesReview();
  const version = builtin.versions[10]!;
  assert.equal(version.id, builtinWorkflowVersionId("no-mistakes-review", 11));
  assert.equal(version.version, 11);
  assert.equal(version.sourceDraftRevision, 10);
  assert.deepEqual(
    version.graph.nodes.map((node) => [node.id, node.kind, node.position.x, node.position.y]),
  ...
  // The snapshot is THIS build's document, which is what keeps the shipped workflow from
  // reporting itself outdated against the shipped Persona it was assembled from.
  const design = version.graph.nodes.find((node) => node.id === "nmr-code-design");
  assert.ok(design && design.kind === "persona");
  const shippedDesign = BUILTIN_PERSONAS.find((item) => item.id === "builtin:code-design-reviewer")!;
  assert.equal(design.persona.sourcePersonaId, shippedDesign.id);
  assert.equal(design.persona.guidanceMarkdown, shippedDesign.guidanceMarkdown);
  assert.equal(personaSnapshotIsOutdated(design.persona, shippedDesign), false);

  const route = (source: string, port: string) => version.graph.edges
    .filter((edge) => edge.source === source && edge.sourcePort === port)
    .map((edge) => `${edge.target}:${edge.targetPort}`)
    .sort();
  // Design is activated by the same pass that activates the other two, so all three judge one
  // submission and one repair packet carries whatever they found.
  assert.deepEqual(route("nmr-intent-conformance", "pass"), [
    "nmr-code-design:activate",
    "nmr-code-quality-judge:activate",
    "nmr-code-risk:activate",
  ]);
  for (const nodeId of ["nmr-code-risk", "nmr-code-quality-judge", "nmr-code-design"]) {
    assert.deepEqual(route(nodeId, "pass"), ["nmr-depth-join:result"]);
    assert.deepEqual(route(nodeId, "fail"), ["nmr-depth-join:result"]);
  }
  assert.deepEqual(route("nmr-depth-join", "fail"), ["nmr-session:return_for_changes"]);
  ...
  // Everything except stage 3's third member is version 10's posture, unchanged.
  assert.deepEqual(version.completionPolicy, { kind: "none" });
  assert.equal(version.resumptionPolicy, "auto");
...
test("appending version 11 rewrote no earlier version", () => {
  ...
  for (const [index, version] of builtin.versions.slice(0, 10).entries()) {
    assert.equal(
      ...
      false,
      `version ${index + 1} grew Code Design Reviewer`,
    );
  }
  assert.deepEqual(
...
test("version 13 changes only evidence readiness enforcement", () => {
  const builtin = noMistakesReview();
  const prior = builtin.versions[11]!;
  const version = builtin.versions[12]!;
  assert.equal(version.id, builtinWorkflowVersionId("no-mistakes-review", 13));
  assert.equal(version.version, 13);
  assert.equal(version.sourceDraftRevision, 12);
  assert.equal(version.evidenceReadinessPolicy, "criterion_mapped_v1");
  assert.equal(prior.evidenceReadinessPolicy, "off");
  assert.deepEqual(version.graph, prior.graph);
...
test("the review presets ship the agreed judges and delivery boundaries", () => {
  const expected = {
    "general-review": [
      ["check:typecheck", "check:test", "check:lint"], ["builtin:intent-conformance-judge"],
...
test("version 18 adds only the Inspector gate to version 17", () => {
  ...
  assert.deepEqual(versions[17], {
    ...versions[16]!,
    id: builtinWorkflowVersionId("no-mistakes-review", 18),
    version: 18,
  ...
  assert.deepEqual(versions[16]!.completionPolicy, { kind: "none" });
  assert.deepEqual(versions[17]!.completionPolicy, {
    kind: "inspector", onFindings: "inspector_only", missingPrAction: "wait",
  });
});

for (const [slug, versionNumber, lintId, historicalDigest] of [
  ["no-mistakes-review", 19, "nmr-check-lint",
    "de90dd1233bb86bbff1e822632ed3b080753c72b443271801f2d9f7d37eb9cbe"],
  ["general-review", 2, "gr-lint",
    "98650066089e1ad0b80d9e31f1d50f7a9203748cebe6727f2a1e43a6fe87a945"],
  ["bug-fix-review", 2, "bfr-lint",
    "92b3439ef78b9b2159dd6ed42679ac5aa110c08e7d220965d65de172745dd032"],
] as const) {
  test(`${slug} gates every reviewer on typecheck, test, and lint in its first stage`, () => {
    ...
    const route = (source: string, port: string) => current.graph.edges
      ...
      .map((edge) => `${edge.target}:${edge.targetPort}`)
      .sort();
    assert.deepEqual(route(pipeline.sessionId, "submitted"),
      gate.members.map((member) => `${member.nodeId}:activate`).sort());
    for (const member of gate.members) {
      assert.ok(member.nodeId);
      for (const port of ["pass", "fail"]) {
        assert.deepEqual(route(member.nodeId, port), [`${gate.joinId}:result`]);
      }
    }
    assert.deepEqual(route(gate.joinId, "fail"), [`${pipeline.sessionId}:return_for_changes`]);
    assert.deepEqual(route(gate.joinId, "pass"),
      evaluation(pipeline.stages[1]).members.map((member) => `${member.nodeId}:activate`).sort());
