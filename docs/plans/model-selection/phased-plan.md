...
- **Both latent bugs are fixed in this work**: the Inspector's env-dropping fallback and the
  workflow-context provenance mislabelling. Review then surfaced a third, verified against the
  current tree: a Foreman role model saved while Foreman's own provider is unset is stranded by an
  app-wide provider change, because Foreman's panel only clears on its own select. Phase 2 fixes it.
...
| Tolerant read, strict write | Phase 1 | Phase 2 | A stored provider is permissive on read so `resolveLlmRunner` can report an unreadable **string** id as `unknown`, and strict on write so a bad value is a 400. The `.catch` sits on the **value**: on the record one bad entry discards the rest, absent one bad entry throws the whole config |
...
| Pin-on-provider-change | Phase 1 | Phase 2 | A convenience, not the invariant: changing a group-level provider first materialises the outgoing one onto everything below it that has a model and no provider. Each blob's own writer does it - `setLlmConfig` for `llm`, `setForemanConfig` for `foreman`. No cross-blob writes |
...
- A Foreman role model saved while Foreman's own provider is unset survives an app-wide provider
  change - the one behaviour on this list that is broken before the work starts.
