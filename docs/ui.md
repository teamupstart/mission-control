...
| <kbd>⌃</kbd><kbd>G</kbd> | [Open what you are writing in a full-size editor](sessions.md#writing-a-longer-message). <kbd>⌘</kbd><kbd>Enter</kbd> or <kbd>⌃</kbd><kbd>Enter</kbd> puts the result back in the send box **unsent**; <kbd>Esc</kbd> discards it. Fires from inside the box, so it can only ever be bound to a <kbd>⌘</kbd>/<kbd>⌃</kbd> combination or a function key, never a text-editing command or a <kbd>⌃</kbd><kbd>⌥</kbd> pair - anything else would be typed into the message | The conversation's reply box |
