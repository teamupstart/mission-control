...
| TP-04 | Not now, the close button, Escape, and clicking the backdrop dismiss the picker without changing the underlying route or saved startup preference. It stays closed for that document. |
...
- Desktop: approximately 880px dialog width, a 300px tour list, flexible preview, and a footer containing the checkbox and Not now. Use max-width/max-height against the viewport and scrollable content; keep close and primary actions reachable.
