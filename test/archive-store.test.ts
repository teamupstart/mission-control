...
import { ARCHIVE_SEARCH_LIMITS, decodeArchiveCursor, archiveKey } from "../src/shared/archives.ts";
...
async function index(store: InstanceType<typeof ArchiveStore>, spec: Parameters<typeof writeScoutBundle>[1], epoch = 1) {
    ...
    db
      .prepare(
        `SELECT name FROM sqlite_master
          WHERE type = 'table' AND (name = 'archives' OR name LIKE 'archive\\_%' ESCAPE '\\')
          ORDER BY name`,
      )
      .all() as unknown as Array<{ name: string }>
  ).map((row) => row.name);
  // `archive_capture_jobs` is here too, and it is a different kind of table: local
  // coordination for archives still being written, not a projection of the library. It
  // arrives on the same additive upgrade path and, like the other three, holds no foreign
  // key to anything.
  assert.deepEqual(tables, [
    "archive_artifacts",
    "archive_capture_jobs",
    "archive_search_segments",
    "archives",
  ]);
});

test("no archive table references a task or a session", () => {
  for (const table of [
    "archives",
    "archive_artifacts",
    "archive_capture_jobs",
...
test("pagination is newest-first, stable, and cursor-bounded", async () => {
  ...
  const second = store.list({ ...EMPTY_QUERY, limit: 2, cursor: decodeArchiveCursor(first.nextCursor) });
  assert.deepEqual(second.rows.map((row) => row.key), newestFirst.slice(2, 4));
  const third = store.list({ ...EMPTY_QUERY, limit: 2, cursor: decodeArchiveCursor(second.nextCursor) });
...
test("archives sharing a completion instant still have a total order", async () => {
  ...
  const second = store.list({ ...EMPTY_QUERY, limit: 1, cursor: decodeArchiveCursor(first.nextCursor) });
...
test("the list limit is bounded at the store, not only at the route", async () => {
  ...
  assert.ok(page.rows.length <= ARCHIVE_SEARCH_LIMITS.maxLimit);
});
