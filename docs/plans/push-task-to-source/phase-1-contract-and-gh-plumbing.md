...
4. `src/server/task-sources/github-issues.ts`
   ...
   - `push()` deliberately does NOT read `ctx.signal` - there is no equivalent of the sweep's post-run aborted check (`github-issues.ts:181`) - because `run()` has no cancellation parameter and `GH_TIMEOUT_MS` is the bound; the parameter exists only for `SweepContext` shape parity. Document this on the function. If a post-run aborted check is ever added, an abort observed AFTER the subprocess ran must read as `outcomeUnknown: true` (the create may have landed), never as a refusal - the sweep can map aborted to a plain error only because a sweep retracts nothing.
...
- `push()` accepts `ctx.signal` for shape parity but does not read it: callers must not rely on the signal to interrupt anything. The bounds on a wedged push are `GH_TIMEOUT_MS` and phase 2's in-flight guard.
...
- 2026-08-09 (Inspector round 1, PR #462): made explicit that `push()` does not read `ctx.signal` and why, and pinned the rule for any future aborted-after-run check (`outcomeUnknown`, never refusal). Pairs with phase 2 dropping its `AbortController` "escape hatch" framing - `run()` has no cancellation parameter, so no phase may describe a cancellation path.
