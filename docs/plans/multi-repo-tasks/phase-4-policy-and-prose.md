...
- Session-action prompt: `docs/session-actions/pull-request.md` (regenerate via `npm run session-actions`; never hand-edit the generated module).
...
- Follow-through: `src/server/foreman/review-followup.ts` - `FollowupMark { prKey, findingsRound, ciNudged }` is one mark per session (`:73-80`), so a new PR resets the sibling's nudge history; the gate at `:155` requires the scalar `session.prUrl`; the payload at `:249` instructs "Do NOT open a new pull request - push your fixes to this same branch." That instruction is correct per PR and wrong per session once a session legitimately owns N PRs.
- Prose with single-PR assumptions (from scoping): `skills/pull-request/SKILL.md:32-37` ("Open a new pull request... verify its current description"); `skills/phased-plan/SKILL.md:33-34,137-148,200-239` (one phase = one PR = one repo, rooted at `process.cwd()`); `docs/session-actions/pull-request.md:5-11` ("turn the work... into one open pull request... Do this once."); `AGENTS.md:53` ("monitor the existing PR").
- Generated files contract: built-in session actions are generated from `docs/session-actions/*.md` via `npm run session-actions`, with a drift test importing the generator.
...
3. **`docs/session-actions/pull-request.md`** + `npm run session-actions`: the action's prompt is delivered per run, and phase 3 made runs per-repo - rewrite "Do this once" to bind to the run's repository ("one open pull request for this repository and branch; update it rather than opening a second one for the same branch").
