...
2. The manager validates the current external-artifact expectation and captures the already-reserved image/text evidence. Any failure discards the candidate. Once those guards pass, the owner atomically promotes or verifies the digest-level artifact and registers a provisional submission claim before persisting raw context. One daemon-owned `WorkflowStore.commitRepositoryCaptureActivation` method activates the claim and marks the submission `running` in the same SQLite transaction; a failure on either write rolls back both. Any intervening failure releases the provisional claim, and startup reconciliation releases it after a crash if the submission never reached `running`.
...
Use a private temporary index, modeled on the ensemble snapshot implementation, so capture never mutates the operator's real index. Candidate preparation happens inside `captureStableWorkflowContext`: allocate a random candidate id in a bounded daemon-owned pending namespace, atomically write a marker containing candidate id, root kind, creation time, and capture nonce before artifact bytes, sample the boundary, build candidate objects and manifest, resample status and identity, and return an unclaimed candidate only when both samples agree. `captureAndActivate` owns the later commit point. It discards the exact marked candidate if external-artifact validation or reserved evidence capture fails, otherwise atomically promotes or reuses the digest-level artifact record and inserts a provisional submission claim before raw context persistence. The single `WorkflowStore.commitRepositoryCaptureActivation` owner activates that claim and marks the submission `running` in one transaction; injected failure on either write must leave both states unchanged. Every earlier post-promotion failure or cancellation releases the provisional claim, and startup reconciliation releases a crash-stranded provisional claim. Clean abandoned candidates and temporary indexes on every mismatch or failed guard.
...
Materialization is bound to the active workload attempt, not merely to provider-supplied locator/digest values. The trusted workload supervisor constructs a `RepositoryMaterializationRequest` carrying submission, workload, Workflow attempt, locator, and digest identity only after matching all five values to the active `PersonaWorkloadRequest`. Before filesystem access, the artifact service independently verifies that the submission still owns an active claim for the same ready digest/locator. Any mismatch fails closed without creating a lease or materialized view.
...
Each submission owns a durable provisional or active claim, not the shared artifact bytes, and every retry for that submission reuses its active claimed digest. Retention, submission deletion, or pre-activation failure atomically marks that claim released and enqueues digest cleanup only when no provisional or active submission claim remains. The retryable daemon-owned reconciler rechecks the zero-claim invariant in the deletion transaction before removing bytes and the digest record. Startup reconciliation repairs database/filesystem non-atomicity, releases provisional claims whose submissions never atomically reached `running`, restores or retires other claims conservatively, and deletes only zero-claim artifacts in the dedicated Workflow namespace whose digest ownership is proven. It separately scans only immediate children of the bounded pending namespace; because no unpromoted candidate has a durable consumer after restart, it removes every candidate whose containment and atomic marker agree, while quarantining or reporting malformed and unowned entries. Periodic cleanup consults the in-memory active-candidate registry and cannot remove a live capture.
...
Every MCP response includes a daemon-minted operation-instance id, separate closed operation kind, status, structured result metadata, byte and item counts, `truncated`, an explicit `nextCursor` when more data is available, and a typed error or denial code. Each successful content-bearing result item also carries an opaque `evidenceHandleId`. The MCP emits matching safe handle metadata containing the canonical snapshot/workload/attempt/operation-instance/kind identity, item ordinal, approved path, half-open line/raw-byte/old-new-diff range, policy version, and truncation state, but no excerpt or response body. Pagination cursors are opaque, integrity-checked tokens bound to snapshot digest, operation shape, and prior position so a provider cannot turn one cursor into a different request.
...
Add an append-only `workflow_repository_query_events` ledger keyed by attempt id and MCP operation sequence, plus bounded evidence-handle metadata keyed by opaque handle id. Each query record includes workload id, workload event sequence, snapshot digest, daemon-minted operation-instance id, closed operation kind, safe path or query hash metadata, start/end time, duration, outcome, denial/error code, item and byte counts, truncation, cursor presence, and cancellation state. Each handle record uses the canonical snapshot/workload/attempt/operation-instance/kind/item/path identity and half-open line/raw-byte/old-new-diff range contract defined below. Do not store repository response bodies, excerpts, quote fields, sensitive query text, or provider secrets.
...
Append a metadata-only `repository` kind to the existing stable evidence-kind vocabulary. Refactor `EvidenceRef`, `WorkflowEvidenceRefSchema`, `EvidenceInputSchema`, verdict normalization, audit helpers, and renderers to one discriminated union. The existing eight kinds retain their current identifiers and quote/path/line wire shapes. The new repository branch contains only a daemon-minted operation-instance id and `evidenceHandleId`, with no quote, excerpt, or provider-supplied path/range field. One canonical daemon-owned handle contract binds snapshot digest, workload id, Workflow attempt id, operation-instance id, separate closed operation kind, returned item ordinal, canonical approved path, policy/truncation state, and exactly one half-open range: `line` is 1-based `startLine`/`endLineExclusive` over LF-delimited text, `byte` is 0-based `startByte`/`endByteExclusive` over immutable raw blob bytes with `encoding: "raw"`, and `diff` carries separate 1-based half-open old/new line intervals with equal bounds allowed for an empty side. Every producer, event parser, store, engine validator, provider adapter, and renderer uses this contract. The engine accepts a handle only when it was minted for a successfully returned item from the same bound identities and operation kind. A truncated item remains bound to only the exact returned range. A fabricated, duplicate-conflicting, denied, failed, cancelled, or unrelated handle is invalid. Repository artifacts do not count toward criterion-mapped submission evidence readiness. Existing prompt-only evidence remains valid for Personas with `none` access and for old runs. Ordinary reviewer prose remains part of the existing bounded/scrubbed final verdict, but it is not treated as a verified repository excerpt.
...
Repository citations have an explicit two-step versioned rollout gate. First ship a writer-disabled compatibility-floor build that adds a hard startup refusal when `PRAGMA user_version` exceeds `CURRENT_DATABASE_SCHEMA_VERSION` and can parse, preserve, and render the repository evidence branch as unsupported metadata without launching repository workloads. Only after the sole daemon owning each state home and every configured executor report that floor or newer may a later activation enable citation writers; the daemon, workload executor, and verdict parser must still advertise the same repository-evidence protocol capability before each read-enabled workload launches. A missing capability, below-floor reader, or unknown newer database schema fails before provider launch or durable verdict write. Normal rollback disables repository dispatch/writes and runs the compatibility-floor build against the live additive schema, preserving repository citations and unrelated post-migration writes. The verified pre-migration recovery point is disaster recovery only, not routine rollback. An exceptional downgrade below the floor stops the daemon, copies and verifies the live database, exports repository-owned rows for idempotent replay, transactionally removes only repository-owned references/state while retaining unrelated writes, lowers the schema marker only after a frozen-target-reader verification succeeds, and replays the export on re-upgrade.

Migration and mixed-version tests open representative pre-feature databases, including custom Personas, built-in workflow history, published custom workflows, runs, attempts, and LLM call audit rows. They prove that all old records parse as `none`, repeated migration is safe, old versions keep identity, identical republish is idempotent, a changed resolved Persona snapshot creates a new immutable workflow version, old eight-kind verdicts remain readable, canonical repository ranges round-trip through every current schema, newer schemas are refused before mutation, and capability mismatches are rejected before dispatch or verdict persistence. A frozen compatibility-floor reader preserves repository citations without executing them; feature-disable rollback retains unrelated post-migration writes; and the exceptional targeted downgrade/export/replay procedure passes a frozen target-reader fixture.
...
- Evidence handles are minted only for actually returned allowed items; canonical 1-based half-open line, 0-based half-open raw-byte, and independent old/new diff intervals round-trip exactly, while forged operation instances, kind mismatches, cross-attempt, mismatched-range, denied, failed, cancelled, and conflicting duplicate handles fail without persisting or reconstructing a quote.
...
- Snapshot capture, portable reconstruction, and rollback under checkout mutation, Git failure, database failure, digest mismatch, every prepare/promote/activation-commit/release/discard crash window, failure of either activation write, restart reconciliation, retention, and cleanup retries.
- Per-repository run capture resolves the same checkout as `workflowCheckoutPath`; an external-artifact mismatch or reserved-evidence failure leaves no claim or durable raw context, and every failure/crash after promotion but before activation releases or reconciles the provisional claim.
...
- Mixed-version repository-evidence capability negotiation and database reader fencing fail before provider launch or durable verdict write; the compatibility-floor reader preserves old evidence plus canonical repository ranges, feature-disable rollback preserves unrelated live writes, and targeted downgrade/replay passes frozen-reader verification.
...
- Both provider paths return and replay line-window, byte-window, and diff items with identical discriminated evidence-handle range metadata.
...
- Identical artifacts may share digest-level bytes, but each submission has a durable provisional or active claim; releasing one submission cannot delete bytes until the last unreleased claim is released and cleanup rechecks that invariant atomically.
