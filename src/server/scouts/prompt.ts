...
export function scoutReportAppendix(slots: readonly ArchiveRepoSlot[]): string {
  const lines = [
    ...
    "so anything worth keeping still belongs in the report.",
    "",
    ...
    "   Do NOT commit the report or other scout output unless the operator's task explicitly requests",
    "   a commit. Do NOT open a pull request for the report. A scout ships an answer, not a change.",
    "",
