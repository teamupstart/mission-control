...
2. create/reserve the provider Engineer run with the commission id as opaque correlation;
3. persist the returned Engineer run id;
4. only then launch the selected host;
5. emit task/commission state after durable writes.
...
Capability mismatch, create failure, malformed JSON, or missing required host support fails before spawn with an actionable error. Retrying reuses the same commission/run idempotently rather than creating duplicates.
...
- authoring run and step history;
- tier/track/skips;
...
Cancellation routes through the provider capability during authoring and existing provider control during implementation. Stale later events cannot reopen a cancelled commission. Host exit alone marks interruption, not step failure.
...
- idempotent task retry and collision refusal;
