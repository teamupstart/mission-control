...
The retained set is the only valid revision domain for `git_show`, `git_log`, and `git_blame`. Materialize frontier metadata as a shallow boundary or equivalent closed reader structure. `git_log` stops at the frontier; `git_blame` marks boundary attribution and truncation; `git_show` metadata is available at a retained frontier, its fixed patch compares with the recorded first parent, a true root compares with the empty tree, and a non-root frontier whose first parent is omitted returns `history_boundary`. Requests for any other revision return `revision_out_of_range` even when its object happens to be present as the source-base anchor. Never fetch or consult the original repository to extend the range.
...
- `git_show`, `git_log`, and `git_blame` work inside the retained set, report frontier behavior without misleading root patches, and reject reachable, source-base-only, and arbitrary revisions outside the set as `revision_out_of_range`;
