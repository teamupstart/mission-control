...
3. Add `membersReady: number` to `EnsembleSummary`: the count of members who own a ready artifact AND are not counted in `membersNeedingInput` AND are not counted in `membersOut`. This is deliberately member-level and MUTUALLY EXCLUSIVE, unlike `readyArtifacts` (which counts artifacts and stays untouched): a `submitted` member with a pending review is one member, and a projection rendering `readyArtifacts` + the blocked count would show them twice and can exceed `maxMembers` (Inspector round 7). It needs both a store fact (which members own ready artifacts - a bounded per-run query the store exposes) and a registry fact (needs input), so it is computed in the manager decoration beside `membersNeedingInput`. The published counts `membersOut` / `membersNeedingInput` / `membersReady` are PAIRWISE DISJOINT - no member is in two of them - but they are not a partition: an active or launching member with no ready artifact and no pending input is in none, and "working" is always the remainder a consumer computes (`launchedMembers` minus the three).
4. Add `needsInput: boolean` to `TaskEnsembleLink` with the same derivation note as `membersNeedingInput`.
5. Widen `ensembleNeedsAttention` (currently `{status, unreadable}`, `ensemble.ts:1260-1268`) to accept an optional `membersNeedingInput?: number` and return true when it is > 0. Optional with a 0 default so existing callers compile unchanged. `membersOut` does NOT feed attention - member failure already has its terminal-run and retry surfaces, and the barrier-impossible path fails the run.
6. Add `ensembleStageWord(summary: Pick<EnsembleSummary, "status" | "outcomeKind">): string` - a pure total map: `planning` -> "launching", `running` -> "working", `waiting` -> "waiting", `evaluating` -> "reviewing", `awaiting_decision` -> "waiting on you", `finalizing` -> "promoting", `cancelling` -> "cancelling", `completed` -> "done", `cancelled` -> "cancelled", `failed` -> "failed", null/unreadable -> "unreadable". It reads only the summary (the compiled plan is not on the wire, deliberately).
...
4. Add the invalidation edge in the existing registry subscription (the block that early-returns at `manager.ts:269-270`): on `session_upsert`, if the session's task id maps to a known member (consult the links map - it is already keyed by taskId), recompute that member's `needsInput` and that run's summary counts, and when either changed from the cached previous value: FIRST update the links map for the affected taskId (rebuild that entry, or call `refreshLinks()` - the summary republish alone leaves the link's `needsInput` stale until an unrelated task event, because `refreshLinks` is otherwise triggered only by `store.onTaskLinksChanged`), THEN `refreshProjection()` so `resyncSessionTask` pushes the fresh link onto the session, THEN `publish(runId)` for the summary. The change-guard covers BOTH derived values (the per-run counts and the per-member `needsInput`) and is mandatory: `publish` -> `refreshProjection` -> `resyncSessionTask` -> `session_upsert` would otherwise loop. Cache the last-published counts per run and last `needsInput` per member inside the manager; the cache is both the loop guard and the staleness detector.
...
- Update `test/ensemble-sse.test.ts:135` exact-keys list (insert `"membersNeedingInput"`, `"membersOut"` and `"membersReady"` in sorted position between `"memberCount"` and `"outcomeKind"`).
...
- Disjointness test in `ensemble-needs-input.test.ts`: a `submitted` member with a pending review counts in `membersNeedingInput` and NOT in `membersReady`; resolving the review moves them (1 blocked / 0 ready becomes 0 / 1); `membersOut + membersNeedingInput + membersReady <= launchedMembers` holds across every driven state (strict inequality whenever a plain working member exists).
...
- New `test/ensemble-needs-input.test.ts` (comment: what is at stake is the fleet and the run agreeing about a blocked member): drive a stub registry + store through the manager - (a) creating a review on a member session republishes the summary with `membersNeedingInput: 1` and `attention: true`; (b) resolving it returns to 0; (c) the republish is edge-guarded (a second identical `session_upsert` emits nothing - count `ensemble_upsert` events); (d) the link is fresh ON THE SESSION EDGE: after the review-created `session_upsert` alone - no task event in between - the projection the registry holds returns `needsInput: true` for that member's taskId, and the member session's own `session_upsert` from `resyncSessionTask` carries it (this is the round-9 regression: summary republished but chip stale); (e) the boot path (`listSummaries`) reports the same decorated value as `publish`; (f) a `retained` member's pending review does not count.
...
- 2026-07-26 (round 9): the session-edge handler now updates the affected link BEFORE refreshing the projection, after the Inspector showed the summary republish alone left `TaskEnsembleLink.needsInput` stale until an unrelated task event triggered `refreshLinks`. The edge guard's cache now covers both derived values, and test (d) pins link freshness on the bare session edge.
- 2026-07-26 (round 8): "partition" corrected to "pairwise disjoint plus a working remainder" - a working member is in none of the three counts, which is exactly why Phase 3 computes `working` as a remainder. Wording only; the disjointness contract is unchanged.
- 2026-07-26 (round 7): added `membersReady` as the member-level, mutually exclusive complement after the Inspector showed `readyArtifacts` + `membersNeedingInput` double-counts a submitted member with a pending review, letting a dot rendering exceed `maxMembers`. The three published member counts are pairwise disjoint; `readyArtifacts` keeps its artifact-count meaning untouched.
